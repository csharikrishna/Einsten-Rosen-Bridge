 ...
 */
 import { DigitalGlitch } from 'https://unpkg.com/three@0.128.0/examples/jsm/shaders/DigitalGlitch.js';
...
export function initScene() {
   return new Promise((resolve) => {
       ...
       function createCachedTexture(key, textureFunction) {
           if (!textureCache[key]) {
               textureCache[key] = textureFunction();
               // Apply texture optimization
               textureCache[key].anisotropy = renderer.capabilities.getMaxAnisotropy();
               textureCache[key].generateMipmaps = true;
               textureCache[key].needsUpdate = true;
           }
           return textureCache[key];
       }
       
       // Cached texture creators
       function getNebula() {
           return createCachedTexture('nebula', createNebulaTexture);
       }
       
       function getGalaxy() {
           return createCachedTexture('galaxy', createGalaxyTexture);
       }
       
       function getEventHorizon() {
           return createCachedTexture('horizon', createEventHorizonTexture);
       }
       
       // ======== UTILITY FUNCTIONS ========
       // Texture generation functions from original code
       function createNebulaTexture() {
           ...
           // Generate nebula-like appearance with fewer iterations for performance
           const colors = [
               'rgba(50, 0, 80, 0.5)',
               'rgba(0, 30, 100, 0.5)',
               'rgba(0, 50, 80, 0.5)',
       ...
       function createGalaxyTexture() {
           ...
           gradient.addColorStop(0.3, 'rgba(100, 100, 200, 0.6)');
           gradient.addColorStop(0.6, 'rgba(50, 50, 150, 0.3)');
           gradient.addColorStop(1, 'rgba(0, 0, 50, 0)');
           
           ctx.fillStyle = gradient;
           ctx.fillRect(0, 0, canvas.width, canvas.height);
           
           // Draw spiral arms (optimized for performance)
           for (let a = 0; a < Math.PI * 2; a += Math.PI / 2) {
               for (let r = 0; r < canvas.width / 2; r += 8) { // Increased step size
                   const armWidth = 20 + r / 5;
                   const spiral = a + r / (canvas.width / 2) * Math.PI * 2;
                   
                   const x = centerX + Math.cos(spiral) * r;
                   const y = centerY + Math.sin(spiral) * r;
                   
                   const opacity = (1 - r / (canvas.width / 2)) * 0.7;
                   ctx.fillStyle = `rgba(200, 200, 255, ${opacity})`;
                   
                   ctx.beginPath();
                   ctx.arc(x, y, armWidth * Math.random(), 0, Math.PI * 2);
                   ctx.fill();
               }
           }
           
           // Add stars
           for (let i = 0; i < 200; i++) { // Reduced star count for performance
       ...
       function createEventHorizonTexture() {
           ...
           gradient.addColorStop(0.3, 'rgba(0, 150, 255, 0.5)');
